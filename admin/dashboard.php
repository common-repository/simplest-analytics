...
foreach ($rows as $row) {
	...
	} else if ($row->track_type == "video") {
		if ($row->event_action !== "") {
			$video_data[$row->event_action][] = $row;
			$tracked_videos[] = $row->event_action;
		}
...
<div class="wrap">
	...
	<nav class="nav-tab-wrapper">
		<a href="?page=simplest-analytics&tab=dashboard<?php echo esc_html($tab_para) ?>"
			class="nav-tab <?php if ($tab === 'dashboard'): ?>nav-tab-active<?php endif; ?>">
		...
		if (is_plugin_active('woocommerce/woocommerce.php')) {
			?>
			<a href="?page=simplest-analytics&tab=woocommerce<?php echo esc_html($tab_para) ?>"
		...
		?>
		<a href="?page=simplest-analytics&tab=rawdata<?php echo esc_html($tab_para) ?>"
			class="nav-tab <?php if ($tab === 'rawdata'): ?>nav-tab-active<?php endif; ?>">
	...
	<div class="tab-content">
		<?php switch ($tab):
			case 'dashboard':
				$tracked_videos = array_unique($tracked_videos);
				$tracked_videos = sizeof($tracked_videos);
				// tab: dashboard
				?>
				<!-- tab: dashboard -->
				<div class="all_tabs" id="tab_dashboard">
					...
					<div class="top_card">
						<span class="spanblock">
							<?php echo esc_html($tracked_events) ?>
						</span>
						<label>
					...
					<div class="top_card">
						<span class="spanblock">
							<?php echo esc_html($tracked_videos) ?>
						</span>
						<label>
							<?php echo esc_html__('Tracked videos', 'simplest-analytics') ?>
						</label>
					</div>
					<!-- END top cards -->
					...
					<script type="text/javascript">
						google.charts.load('current', {'packages': ['table']});
						google.charts.setOnLoadCallback(simplest_analytics_drawTable_video);

						function simplest_analytics_drawTable_video() {
							var data = new google.visualization.DataTable();
							data.addColumn('string', '<?php echo esc_html__('Tracked videos', 'simplest-analytics') ?>');
							data.addColumn('string', '<?php echo esc_html__('event', 'simplest-analytics') ?>');
							data.addColumn('number', '<?php echo esc_html__('Visits', 'simplest-analytics') ?>');
							data.addColumn('number', '<?php echo esc_html__('Unique visitors', 'simplest-analytics') ?>');
							data.addRows([
								...
								if (sizeof($video_data) == 0) {
									...
									foreach ($video_data as $key => $val) {
										...
										foreach ($val as $v) {
											$visits++;
											$session_ids[] = $v->session_id;
											$video_event_data[$v->parameters][] = $v->session_id;
										}
										$unique_visitors = array_unique($session_ids);
										$unique_visitors = sizeof($unique_visitors);
										$event = "all";
										...
										<?php
										// sort events
										$sort_order = ['1sec', '25%', '50%', '75%', '100%'];
										foreach ($sort_order as $sort) {
											if (isset($video_event_data[$sort])) {
												$visits = sizeof($video_event_data[$sort]);
												$unique_visitors = array_unique($video_event_data[$sort]);
												$unique_visitors = sizeof($unique_visitors);
												$event = $sort;
												?>
												[
							...
							var table = new google.visualization.Table(document.getElementById('sa_video_div'));

							table.draw(data, {showRowNumber: true, sortColumn: 0, sortAscending: false, desc: true, width: '100%', height: '100%'});
						}
					</script>
					<div class="chart_table chart_table_3col" id="sa_video_div"></div>
					<!-- END video -->
			...
			case 'woocommerce':
				...
				if (is_plugin_active('woocommerce/woocommerce.php')) {
					...
					<div class="all_tabs" id="tab_woocommerce">
						...
						<div class="top_card">
							<span class="spanblock">
								<?php echo wc_price($sales_amount) ?>
							</span>
							<label>
...
<div id="daterange_popup" class="popup_closed closepopup box-shadow-3">
	...
	<div id="select_ranges">
		...
		<div class="datechoice"
			onclick="simplest_analytics_show_ele('select_custom');simplest_analytics_hide_ele('select_ranges')">
	...
	<div id="select_custom" style="display:none">
		<div class="daterange_head">
			<span onclick="simplest_analytics_hide_ele('select_custom');simplest_analytics_show_ele('select_ranges')"
				class="dashicons dashicons-arrow-left-alt"></span>
			<span class="hlnb">
				<?php echo esc_html__('custom daterange', 'simplest-analytics') ?>
			</span>
		</div>
		<div class="select_custom_date">
			<label>
				<?php echo esc_html__('from:', 'simplest-analytics') ?>
			</label>
			<input name="from" id="from" type="date" value="<?php echo esc_html($from_date) ?>" />
		</div>
		<div class="select_custom_date">
			<label>
				<?php echo esc_html__('to:', 'simplest-analytics') ?>
			</label>
